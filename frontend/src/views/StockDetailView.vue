...
const toggleWatchlist = async () => {
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (result.added) { if (!watchlist.value.includes(code)) watchlist.value.push(code) }
      else { watchlist.value = watchlist.value.filter(c => c !== code) }
    }
...
.stock-code { color: #666; font-size: 18px; margin-left: 10px; }
...
.toss-nav button { background: none; border: none; color: #666; font-size: 17px; font-weight: bold; padding: 15px 5px; cursor: pointer; border-bottom: 3px solid transparent; }
...
.item-bottom { display: flex; justify-content: space-between; margin-top: 10px; font-size: 15px; }

/* ✅ 4. 모달 수정 - 겹침 방지 및 토스 스타일 */
.modal-overlay {
  position: fixed; inset: 0; background: rgba(0, 0, 0, 0.8);
  backdrop-filter: blur(8px); display: flex; justify-content: center; align-items: center; z-index: 3000;
}
.modal-content {
  background: #1c1c1e; width: 400px; border-radius: 28px; padding: 32px; border: 1px solid #333;
}
...
.trade-input {
  background: #2c2c2e; border: 1px solid #3a3a3c; border-radius: 16px; padding: 16px 20px;
  width: 100%; color: #fff; font-size: 20px; font-weight: bold; text-align: right; padding-right: 50px; outline: none;
}
